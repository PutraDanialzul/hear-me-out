...
export default function OldConfessionMenu(){
    ...
    async function updateData(){
        ...
        const select = await supabase.from("confession").select("created_at, text, id, confessor_id").eq("confessor_id", id).order("created_at", {ascending: false}).range(page * 4, (page+1) * 4 - 1);
    ...
    useEffect(()=>{
        async function initialization(){
            ...
            const id = user.data.user.id;
            const select = await supabase.from("confession").select("created_at, text, id, confessor_id").eq("confessor_id", id);
